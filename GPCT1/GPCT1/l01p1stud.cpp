...
void poligon(Punct centru, double raza,int nr_lat,int fill, int line) {
    std::vector<Punct> puncte;
    const double pi = 3.141;
    const float dim_unghi = 2 * pi / nr_lat;

    for (int i = 0; i < nr_lat; i++) {
        double unghi = (i)*dim_unghi;
        Punct varf = Punct(centru.x + raza * cos(unghi), centru.y + raza* sin(unghi));
        puncte.push_back(varf);
    }
   
    if (fill == 1 && line == 0) glPolygonMode(GL_FRONT, GL_FILL);
    if (fill == 0 && line == 1 ) glPolygonMode(GL_FRONT, GL_LINE);
    glBegin(GL_POLYGON);
    for (auto varf : puncte) {
        glVertex2f(varf.x, varf.y);
    }
    glEnd();
...
void Display8() {
    ...
    poligon( Punct(0.0, 0.0), 0.8, 6, 1, 0);

    glColor3f(1.0, 1.0, 1);
    poligon(Punct(0.0, 0.0), 0.45, 6, 1, 0);

    glColor3f(1.0, 0.1, 0.1);
    poligon(Punct(0.0, 0.0), 0.45, 6, 0, 1);
}
